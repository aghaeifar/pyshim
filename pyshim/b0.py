...
def calc_b0_shim(shimmaps_nii:np.ndarray, b0_nii:np.ndarray, masks:list[np.ndarray], lb:np.ndarray, up:np.ndarray):
    '''
    calculate the shim
    ...
    for mask_nii in masks:
        if b0_nii.shape != mask_nii.shape:
            raise ValueError('b0_map and mask must have the same shape')
        # weighting mask 
        mask_data = np.sqrt(mask_nii)
        b0_data   = (b0_nii * mask_data).flatten()
        shimmaps_data = np.reshape(shimmaps_nii * mask_data[..., np.newaxis], (-1, shimmaps_nii.shape[3]))
